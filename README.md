...
从 Microsoft Store 或其他 MSIX 安装包安装时，应用启动需要管理员权限。首次启动会触发 UAC 提示，因为 `usbipd bind/unbind` 需要提升后的权限。
...
- 执行 `usbipd bind/unbind` 需要管理员权限

## 权限说明

- Microsoft Store / MSIX 安装版需要以管理员权限启动
- 这是因为 `usbipd bind/unbind` 需要提升后的 Windows 权限
- Store Listings 的 Description 应明确说明这一权限要求
